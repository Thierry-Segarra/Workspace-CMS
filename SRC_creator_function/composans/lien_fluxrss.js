function addLienFluxRss(i,nbcomp){
    ...
    //console.log(divcomp);
     let add = '<p id="lienrssdiv'+i+compo+'">Nouveau Flux RSS</p>';
     
     document.getElementById('list_composans').innerHTML =  document.getElementById('list_composans').innerHTML + '<button id="id_op'+i+compo+'" onclick="clearInterval(inter),option_element('+i+compo+',`LIENRSS`,`lienrssdiv`)">option</button>'+'<button id="id_sup'+i+compo+'" onclick="clearInterval(inter),suprime_lienRSS_liste('+i+compo+')">suprimer</button><button id="up'+i+compo+'" onclick="ligne_up('+i+nbcomp+','+i+compo+','+nbcomp+','+i+')">up</button><button id="down'+i+compo+'" onclick="ligne_down('+i+nbcomp+','+i+compo+','+nbcomp+','+i+')">down</button><br id="id_br'+i+compo+'"><p id="trai_lienrssdiv'+i+compo+'">Nouveau Flux RSS</p>';
...
function suprime_lienRSS_liste(i){
    ...
    document.getElementById('lienrssdiv'+i).remove();
    document.getElementById('trai_lienrssdiv'+i).remove();
