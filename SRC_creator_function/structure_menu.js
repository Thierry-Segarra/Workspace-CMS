    ...
    if(tablecomp.length != []){
        ...
        for(let c=0;c < tablecomp.length;c++){
            ...
            if(nom == 'IMAGE'){
                ...
                document.getElementById('div'+divnb).innerHTML = document.getElementById('div'+divnb).innerHTML + '<p id="idnomcomp'+divnb+compnb+'">'+nom+'</p><button id="id_op'+divnb+compnb+'" onclick="clearInterval(inter),option_liste('+divnb+compnb+')">option</button>'+'<button id="id_op'+divnb+compnb+'" onclick="addLigneListe('+divnb+','+compnb+')">add ligne</button>'+'<button id="id_sup'+divnb+compnb+'" onclick="clearInterval(inter),suprime_liste('+divnb+','+divnb+compnb+',`'+table2+'`)">suprimer</button><button id="up'+divnb+compnb+'" onclick="up_comp('+divnb+',`'+table2+'`)">up</button><button id="down'+divnb+compnb+'" onclick="down_comp('+divnb+',`'+table2+'`)">down</button><br id="id_br'+divnb+compnb+'">'
            }
            else if(nom == 'LIGNE LISTE'){
                document.getElementById('div'+divnb).innerHTML = document.getElementById('div'+divnb).innerHTML + '<p id="idnomcomp'+divnb+compnb+'">'+nom+'</p><button id="id_op'+divnb+compnb+'" onclick="clearInterval(inter),option_ligne_liste('+divnb+compnb+')">option</button>'+'<button id="id_sup'+divnb+compnb+'" onclick="clearInterval(inter),suprime_liste('+divnb+','+divnb+compnb+',`'+table2+'`)">suprimer</button><button id="up'+divnb+compnb+'" onclick="up_comp('+divnb+',`'+table2+'`)">up</button><button id="down'+divnb+compnb+'" onclick="down_comp('+divnb+',`'+table2+'`)">down</button><br id="id_br'+divnb+compnb+'">'
            }
        }
    }
}