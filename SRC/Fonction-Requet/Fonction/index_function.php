<script>
        ...
        /* src="SRC/Fonction-Requet/menu.js" Bouton B1*/ 
        var linkB1 = `<?php include("Menu-Page/tableau-de-bord.php") ?>`;

        /* src="SRC/Fonction-Requet/menu.js" Bouton B2*/ 
        var linkB2 = `<?php include("Menu-Page/article.php") ?>`;

        /* src="SRC/Fonction-Requet/menu.js" Bouton B3*/ 
        var linkB3 = `<?php include("Menu-Page/media.php") ?>`;

        /* src="SRC/Fonction-Requet/menu.js" Bouton B4*/
        var linkB4 = `<?php include("Menu-Page/page.php") ?>`;

        /* src="SRC/Fonction-Requet/menu.js" Bouton B5*/ 
        ...
        function sup_art(id){
            if ( confirm( "Voulez vous suprimer cette article" ) ) {
                // suprime (true)
               window.location = "./SRC/Fonction-Requet/Suprimer/suprimer_article.php?id="+id;
            } else {
                // rien (false)
            }
        }
        function sup_pag(id){
            if ( confirm( "Voulez vous suprimer cette page" ) ) {
                // suprime (true)
               window.location = "./SRC/Fonction-Requet/Suprimer/suprimer_page.php?id="+id;
            } else {
                // rien (false)
            }
        }
        ...
        function sup_categorie(id){
            if(id != '1'){ // id 1 est la catégorie de base nommé Non-défini
                if ( confirm( "Voulez vous suprimer cette categorie" ) ) {
                    // suprime (true)
                window.location = "./SRC/Fonction-Requet/Suprimer/suprimer_categorie.php?id="+id;
                } else {
                    // rien (false)
                }
            }else{
        ...
        function param_art_pag(id,nb){
            ...
            if(nb == 2){ // Pour la PAGE
                if(document.getElementById('btligneid'+id).innerHTML == ''){
                    ...
                    document.getElementById('btligneid'+id).innerHTML = '<a class="boutton" href="./Menu-Page/creation_page.php?Modification='+id+'">modifier le conteue</a><br><br><div class="L1"><form class="modification" action="SRC/Fonction-Requet/Modifier/modifier_titre_page.php" method="POST"><span>Changer le titre : </span><input type="text" name="titre" value="'+description[id][2]+'" placeholder="Changer le titre" required><input type="text" name="id" value="'+id+'" style="visibility:collapse"><input class="modification" type="submit" value="Modifier Titre"></form><form class="modification" action="SRC/Fonction-Requet/Modifier/modifier_description_page.php" method="POST"><span>Modifier la description : </span><input type="text" name="description" value="'+description[id][0]+'" placeholder="Changer la description"></input><input type="text" name="id" value="'+id+'" style="visibility:collapse"><input type="submit" value="Modifier Description"></form><form class="modification" action="SRC/Fonction-Requet/Modifier/modifier_categorie_page.php" method="POST"><span>Changer de categorie : </span><select name="categorie">'+liste_categorie+'</select><input type="text" name="id" value="'+id+'" style="visibility:collapse"><input type="submit" value="Modifier Categorie"></form></div><button class="boutton" onclick="sup_pag('+id+')"">suprimer</button><button class="boutton" onclick="export_template('+id+',`page`)"">exporter template</button><div id="formexport'+id+'""></div>';
                ...
                if(document.getElementById('btligneid'+id).innerHTML == ''){
                    document.getElementById('btligneid'+id).style.visibility = 'visible';
                    document.getElementById('btligneid'+id).innerHTML = '<a class="boutton" href="./Menu-Page/creation_article.php?Modification='+id+'">modifier le conteue</a><br><br><div class="L1"><form class="modification" action="SRC/Fonction-Requet/Modifier/modifier_titre_article.php" method="POST"><span>Changer le titre : </span><input type="text"  name="titre" value="'+description[id][2]+'" placeholder="Changer le titre" required><input type="text" name="id" value="'+id+'" style="visibility:collapse"><input type="submit" value="Modifier Titre"></form><form class="modification" action="SRC/Fonction-Requet/Modifier/modifier_description_article.php" method="POST"><span>Modifier la description : </span><input type="text" name="description" value="'+description[id][0]+'" placeholder="Changer la description"><input type="text" name="id" value="'+id+'" style="visibility:collapse"><input type="submit" value="Modifier Description"></form><form class="modification" action="SRC/Fonction-Requet/Modifier/modifier_categorie_article.php" method="POST"><span>Changer de categorie : </span><select name="categorie">'+liste_categorie+'</select><input type="text" name="id" value="'+id+'" style="visibility:collapse"><input type="submit" value="Modifier Categorie"></form></div><button class="boutton" onclick="sup_art('+id+')">suprimer</button>';
                    document.getElementById('categorie'+description[id][1]).setAttribute('selected','selected');
                    document.getElementById('bt_fleche'+id).innerHTML = '↑';
                }else{
                    document.getElementById('btligneid'+id).style.visibility = 'hidden';
        ...
        function export_template(id,table){
            let nom_export = prompt("écrire nom du template");
            
            if (nom_export == null || nom_export == "") {

            } else {
            document.getElementById("formexport"+id).innerHTML = '<form action="./SRC/Fonction-Requet/Fonction/exportTemplate.php" method="post" id="export'+id+'"></form>';
            document.getElementById("export"+id).innerHTML = document.getElementById("export"+id).innerHTML+'<input type="text" name="titre" value='+nom_export+'>';
            document.getElementById("export"+id).innerHTML = document.getElementById("export"+id).innerHTML+'<input type="text" name="id" value='+id+'>';
            document.getElementById("export"+id).innerHTML = document.getElementById("export"+id).innerHTML+'<input type="text" name="table" value='+table+'>';
            document.getElementById("export"+id).submit()
            // nom_file = descrip+".json";

            // création du fichier
            // $f = fopen("../JSON/".$nom_file, "r+");
            // // écriture
            // //ftruncate($f, 0);
            // // fputs($f, $texte );
            // // fermeture
