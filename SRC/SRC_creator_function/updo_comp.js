...
function ligne_rss_up(i,comp,nbcomp,divnb){
// i = numero de de la div et du composans
// comp = numero du composans
// nbcomp = numero de la list
// divnb = numero de la div
    ...
    var list = document.getElementById("apercu").innerHTML
    list = list.substring(list.search('<div id="rssdiv'+i));
    list = list.substring(0,list.search('</div>')+6);
    // contenue avant la ligne à monté
    var avant_ligne = list.substring(list.search('<div id="rssdiv'+i)+('<div id="rssdiv'+i+'">').length);
    avant_ligne = avant_ligne.substring(0,avant_ligne.search('<p id="rssliendiv'+comp));

    // isoloer la ligne a monté
    var ligne_selec = list.substring(list.search('<p id="rssliendiv'+comp));
    ligne_selec = ligne_selec.substring(0,ligne_selec.search('</p>')+4);
    
    // contenue apres la ligne à monté
    var apres_ligne = list.substring(list.search(ligne_selec)+ligne_selec.length);
    apres_ligne = apres_ligne.substring(0,apres_ligne.search('</div>'));
    /*
    //console.log('##########');
    //console.log('LISTE : '+list);
    //console.log('|');
    ...
    */
    ...
    if(avant_ligne != ''){
        traitement = avant_ligne;
        while(traitement.search('<p id="rssliendiv')!=-1){ // On veut récuperer le dernier element de la variable avant_ligne donc pas de if
            ligne_deplacer = traitement;
            reste = traitement.substring(traitement.search('</p>')+4);
            av_ligne = av_ligne + traitement.substring(0,traitement.search(reste));
            traitement = traitement.substring(traitement.search('</p>')+4);
            if(traitement == ''){
                document.getElementById('rssliendiv'+i).innerHTML = av_ligne+ligne_selec+ligne_deplacer+apres_ligne;
                document.getElementById("list_composans").innerHTML = '';
                traitement_lignefluxrss(i,divnb,nbcomp)
            }
        }
    }
...
function ligne_rss_down(i,comp,nbcomp,divnb){
    // i = numero de de la div et du composans
    // comp = numero du composans
    // nbcomp = numero de la list
    // divnb = numero de la div
    ...
    var list = document.getElementById("apercu").innerHTML
    list = list.substring(list.search('<div id="rssdiv'+i));
    list = list.substring(0,list.search('</div>')+5);
    // contenue avant la ligne à monté
    var avant_ligne = list.substring(list.search('<div id="rssdiv'+i)+('<div id="rssdiv'+i+'">').length);
    avant_ligne = avant_ligne.substring(0,avant_ligne.search('<p id="rssliendiv'+comp));
    ...
    var apres_ligne = list.substring(list.search(ligne_selec)+ligne_selec.length);
    //console.log('APRES : '+ligne_selec);
    apres_ligne = apres_ligne.substring(0,apres_ligne.search('</p>'));
    ...
    if(apres_ligne != ''){
        traitement = apres_ligne;
        if(traitement.search('<p id="rssliendiv')!=-1){ // On veut récuperer le premier element de la variable apres_ligne donc pas de while
            ...
            if(av_ligne == ''){av_ligne = apres_ligne}
            // affiche le résultat
            document.getElementById('rssdiv'+i).innerHTML = avant_ligne+av_ligne+ligne_selec+traitement;
            document.getElementById("list_composans").innerHTML = '';
            traitement_lignefluxrss(i,divnb,nbcomp)
